...
void init_sliders_attacks(int is_bishop, Bitboard current_occupancy) {
    ...
    for (int square = 0; square < 64; square++) {
        ...
        // Filter current occupancy with the mask for the specific piece
        Bitboard filtered_occupancy = current_occupancy & mask;

        // Calculate the magic index
        Bitboard magic_index = filtered_occupancy * (is_bishop ? bishop_magics[square] : rook_magics[square]) >> (64 - (is_bishop ? bishop_relevant_bits[square] : rook_relevant_bits[square]));

        // Store the attacks based on the current occupancy
        if (is_bishop) {
            bishop_attacks[square][magic_index] = bishop_attacks_on_the_fly(square, filtered_occupancy);
        }
        else {
            rook_attacks[square][magic_index] = rook_attacks_on_the_fly(square, filtered_occupancy);
        }
    }
}
